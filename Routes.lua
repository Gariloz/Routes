...
function Routes:OnInitialize()
	...
	-- Register English command as well
	self:RegisterChatCommand("routes", function() LibStub("AceConfigDialog-3.0"):Open("Routes") end)
	...
	function Routes:InitializeMinimapButton()
		local libDataBroker = LibStub("LibDataBroker-1.1", true)
		local libDBIcon = LibStub("LibDBIcon-1.0", true)
		
		if not libDataBroker then
			print("Routes: LibDataBroker-1.1 not found, minimap button disabled")
			return
		end
		
		if not libDBIcon then
			print("Routes: LibDBIcon-1.0 not found, minimap button disabled")
			return
		end
		
		-- Создаем объект данных для LibDataBroker
		local miniButton = libDataBroker:NewDataObject("Routes", {
			type = "data source",
			text = "Routes",
			...
			OnTooltipShow = function(tooltip)
				if tooltip and tooltip.AddLine then
					tooltip:AddLine("Routes")
					tooltip:AddLine("ЛКМ: Открыть настройки")
				end
			end,
		})

			-- Регистрируем кнопку в LibDBIcon
	-- Убеждаемся, что структура базы данных правильная
	if not self.db.global.minimap then
		self.db.global.minimap = { minimapPos = 200, hide = false }
	end
		libDBIcon:Register("Routes", miniButton, self.db.global.minimap)
	libDBIcon:Show("Routes")
	
	-- Обновляем позицию кнопки
	libDBIcon:Refresh("Routes", self.db.global.minimap)
	

	
	-- Добавляем обработчик для сохранения позиции при изменении
	local button = libDBIcon.objects["Routes"]
	if button and button.db then
		button:HookScript("OnDragStop", function()
			...
			if button.db and button.db.minimapPos then
				self.db.global.minimap.minimapPos = button.db.minimapPos
			end
		end)
	end
...
function Routes:OnEnable()
	...
	Routes:InitializeMinimapButton()
end
